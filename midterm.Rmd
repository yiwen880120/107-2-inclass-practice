...
```{r ans4}
data3
logicPass <- data3$成績 >= 60
logicPass
a <- data3$成績[logicPass]
b <- data3$學分數[logicPass]
passData <- list(
  成績 = a,
  學分數= b
)
...
```{r ans52}
twDates
str_sub(twDates,4,6) -> yearcharacter
yearcharacter
...
str_split(twDates2,"年") -> datesplitlist #用年切開年月值
datesplitlist

map(datesplitlist,~.[[2]]) -> monthlist #選出每個元素第2個值
monthlist
unlist(monthlist) -> monthvector #將list疊成字串向量
monthvector

str_c(as.character(westYears),monthvector,"01",sep = "-") -> westDates #貼成yyyy-mm-dd字串形式
westDates
...
(sampledTranscriptData$科目名稱 == "科目204" & sampledTranscriptData$學期成績 < 60) -> logicChosensubjectandfail
sum(logicChosensubject)
sum(logicChosensubjectandfail)
sum(logicChosensubjectandfail)/sum(logicChosensubject)
# failRate
```
...
head(sampledTranscriptData$學屆) #前六筆
tail(sampledTranscriptData$系碼) #後六筆
...
sampledTranscriptData$學系 <-str_replace(sampledTranscriptData$系級,"[:digit:]","")  #[:digit:]0到9所有數字
# sampledTranscriptData$學系 
```
...
sum(sampledTranscriptData$學期成績[logiSelect]*sampledTranscriptData$學分數[logiSelect])/sum(sampledTranscriptData$學分數[logiSelect]) -> studentGPA
studentGPA
# studentGPA
```
