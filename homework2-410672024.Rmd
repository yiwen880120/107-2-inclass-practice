...
```{r ans12}
names(studentRecord)[[4]][1,2,3,4] <- c("一","二","三","四")
studentRecord
```
...
```{r ans23}
length(commitCheck)
names(commitCheck)[c(1:30)] <- c("commit1":"commit30")
commitCheck
```
...
```{r ans26}
hour_twtime <- hour(allCommitDatesTW)
minute_twtime <- minute(allCommitDatesTW)
logicWithinTimeFrame <- hour_twtime >=4 & hour_twtime <=6  & !(hour_twtime >=6 & minute_twtime >=30 & minute_twtime <=59) 
...
```{r ans31}
as.data.frame(transcriptData) -> variableNames
variableNames
```
...
```{r ans32}
logicvector <- variableNames[,c("學系")] == "經濟學系"
logicvector
```
...
```{r ans34}
transcriptPartialDataEcon <- transcriptData[logicvector][,c(1,7,8)]
